...
export function buildWriterSystemPrompt({ trip, toolRegistry, today }) {
  const days = toolRegistry.listDays.execute();
  const toc = days.map((d) => `${d.day_num}. ${d.date} — ${d.title}`).join('\n');
  return `Jesteś asystentem grupy jadącej na wycieczkę do Toskanii (${trip.meta.start_date} – wyjazd). Dzisiejsza data: ${today}.
Twoje jedyne zadanie: sformułuj PO POLSKU finalną odpowiedź na podstawie danych narzędzi widocznych w historii rozmowy (wiadomości z rolą "tool") oraz spisu dni poniżej. Nie masz dostępu do żadnych narzędzi — nie próbuj ich wołać.
Nic nie zgaduj poza tym co widać w historii narzędzi i w spisie dni: jeśli tam nie ma danych na dany temat, powiedz wprost że ich nie masz.
