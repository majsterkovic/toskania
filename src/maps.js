...
export function initInteractiveMap(containerId, plan) {
  ...
  // invalidateSize() przelicza tylko rozmiar kontenera, NIE ponawia fitBounds —
  // po obrocie telefonu markery zostawały poza kadrem. Ten sam powód co przy
  // timeoutach wyżej, więc i ta sama para wywołań.
  const onResize = () => {
    try { map.invalidateSize(); fitCurrentView(); } catch (_) {}
  };
